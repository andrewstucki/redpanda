...
void unset_all_kubernetes_env_vars() {
    ...
    unsetenv("REDPANDA_METRICS_K8S_CONSOLE_IMAGE_VERSION");
...
BOOST_AUTO_TEST_CASE(test_kubernetes_metrics) {
    ...
    setenv("REDPANDA_METRICS_K8S_CONSOLE_IMAGE_VERSION", "", 1);
    ...
    unset_all_kubernetes_env_vars();
    setenv(
      "REDPANDA_METRICS_K8S_CONSOLE_IMAGE_VERSION",
      ...
      1);
    {
        ...
        BOOST_REQUIRE(km.has_value());
        BOOST_REQUIRE_EQUAL(
          km->console_image_version.value(), "redpanda/console:latest");
    }
    unset_all_kubernetes_env_vars();
