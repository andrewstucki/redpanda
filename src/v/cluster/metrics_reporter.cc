...
std::optional<metrics_reporter::kubernetes_metrics> get_kubernetes_metrics() {
    ...
    if (auto v = std::getenv("REDPANDA_METRICS_K8S_CONSOLE_IMAGE_VERSION");
        v && *v) {
        km.console_image_version.emplace(v);
        any = true;
    }
...
metrics_reporter::build_metrics_snapshot() {
    ...
    if (auto km = get_kubernetes_metrics()) {
...
void rjson_serialize(
  ...
  const cluster::metrics_reporter::kubernetes_metrics& km) {
    ...
    if (km.console_image_version.has_value()) {
        w.Key("console_image");
        w.String(km.console_image_version.value());
    }
